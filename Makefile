...
CXXFLAGS        := $(FLAGS)
LDFLAGS         := $(FLAGS)
INC             := -I $(PROJECT_DIR)/inc -I build/resolvedDep/kent/inc
...
$(EXEC_FILE): $(OBJ)
	$(DIR_GUARD)
	$(LD) $(LDFLAGS) $^ -Wl,-L build/resolvedDep/kent/lib -lmathy -o $@ && echo "[OK]: $@"
	$@
...
$(TEST_EXEC_FILE): $(OBJ_TEST)
	$(DIR_GUARD)
	$(LD) $(LDFLAGS_TEST) $^ -Wl,-L build/resolvedDep/kent/lib -lmathy -o $@ && echo "[OK]: $@"
	$@
